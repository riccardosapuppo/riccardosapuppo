...
> *A dry run and a real run choose the same studies.* Give the dry run its own
> branch, the way it usually gets one, and it failed to mention **9** of the
> 255 studies it was about to delete.
>
...
### [dynamic-form-builder](https://github.com/riccardosapuppo/dynamic-form-builder)

A form is authored as a tree and asked about as a table.
...
> express three of the questions at all. Flattening fixes all of them but the
> rename, which needs an id, and that is the whole finding.

### [patient-portal-suite](https://github.com/riccardosapuppo/patient-portal-suite)

A portal where patients read their own reports, and only their own.
...
> over **70** documents belonging to somebody else, and write **70** audit
> lines saying those documents were handed to their owner.
>
...
### [pacs-analytics-monitoring](https://github.com/riccardosapuppo/pacs-analytics-monitoring)

Analytics over an archive whose schema you do not own and that differs at every
installation.
...
> straight gets **7 answers wrong with nothing to show for it**. They are not
> errors but numbers, four times too high, because a one-to-many join took the
> storage with it.
...
[DICOM viewer, web](https://github.com/riccardosapuppo/medical-dicom-viewer-web)
...
[multi-tenant booking](https://github.com/riccardosapuppo/multi-tenant-booking)
...
[teaching somebody a Windows application](https://github.com/riccardosapuppo/windows-app-guided-tour)
...
[campaigns that cannot write to somebody who said stop](https://github.com/riccardosapuppo/campaign-automation)
· [booking a boat in a system the website does not own, at a price the browser cannot set](https://github.com/riccardosapuppo/charter-booking-bridge)
...
The Node projects have **no runtime dependencies at all** and no build step. The
imports look like `node:sqlite`, `node:http` and `node:fs`, and what Node runs
is the TypeScript itself, with `erasableSyntaxOnly` set so the type check is
also the proof that they still run without one. The C# ones carry no packages
beyond the test runner, and the PHP one carries none beyond it either.
...
It cannot be written at all, and a test asserts that by reflection so it stays
that way.
...
technical challenges of the production solution, with newly written code and
fictional data. No proprietary source code, confidential data or client assets
from an original are included in any of them. What the data describes is
invented in the repository itself: no clinic, no patient, no real archive, no
customer list.

Each README says what the original did, what the reconstruction does
differently, and why.
